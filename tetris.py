...
def check_collision(board, shape, offset):
    #Checks for collisions by checking matrix overlapping
    off_x, off_y = offset
...
class Tetris(arcade.Window):
    ...
    def update(self, dt):
        # Update, drop stone if warrented, *need fix*
        self.frame_count += 1
...
def main():
    ...
    my_game = Tetris(SCREEN_WIDTH, SCREEN_HEIGHT, SCREEN_TITLE)
    my_game.setup()
    arcade.run()
